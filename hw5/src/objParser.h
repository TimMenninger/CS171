...
struct Object
{
    ...
    // The mesh data is another representation using vertices faces and norms
    Mesh_Data         mesh;
    std::vector<HEV*> *hevs;
    std::vector<HEF*> *hefs;

    // Buffers used to render scene
    ...
    void fillNormals();
    void fillBuffers();
    void fillBuffers(std::vector<HEF*> *hefs);
};
...
void fillNormals (std::vector<HEV*> *hevs);
