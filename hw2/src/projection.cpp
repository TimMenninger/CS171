...
/*
 ...
 Takes a world of shapes and transforms them all to camera space.

 Arguments: camera cam - The camera that defines camera space orientation
            vector<shape3D> shapes - Vector of shapes in world space to be
                transformed to camera space.
            vector<shape3D> *camShapes - Vector filled with transformed
                shapes.
...
)
{
    ...
    for (; s != shapes.end(); ++s) {
        ...
        // Convert the shape to Cartesian NDC, removing facets not in
        // camera space
        s->worldToCartNDC(toCam, toNDC, &proj);
